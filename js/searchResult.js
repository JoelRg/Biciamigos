 ...
 * searchResult
 */
function searchResult(data, map) {
 ...
 * Constructor
 */
searchResult.prototype._construct = function(data, map) {
    var thisO = this;
 ...
 */
searchResult.prototype.init = function(data) {
    var thisO = this;
    ...
    $.each(thisO._map._zoomLevels, function(index, zoom) {
        ...
        var idDiv = "search-result-"+ data.id +"-"+ zoom;
        $(thisO._map._mapObjectContainerObjectList[zoom])
            .append("<div id='"+ idDiv +"' class='search-result'></div>");
 ...
 *
 */
searchResult.prototype._initEvents = function() {
    var thisO = this;

    // Tooltip para cada zoom
    $.each(thisO._map._zoomLevels, function(index, zoom) {

        var text = "<div class='searchResult-tooltip-text "+ thisO._id +"' data-bkrid='"+ thisO._id +"'>"
        ...
        thisO._objects[zoom].on('click', function(){
            ...
            var $tooltips = $(".searchResult-tooltip-text", thisO._map._mapObjectContainerObjectList[zoom]);
 ...
 */
searchResult.prototype.show = function(zoom, showTooltip) {
    var thisO = this;
 ...
 * o para todos los zoom si no se especifica
 */
searchResult.prototype.hide = function(zoom) {
    var thisO = this;
 ...
 */
searchResult.prototype.getCoordinates = function(zoom) {
    var thisO = this;
 ...
 */
searchResult.prototype.showTooltip = function(zoom) {
    var thisO = this;
    ...
    if (!zoom) {
        ...
        $.each(thisO._map._zoomLevels, function(index, zoom) {
            ...
            //console.log(thisO._id, $(".searchResult-tooltip-text."+ thisO._id).is(":visible"));
            //if (!$(".searchResult-tooltip-text."+ thisO._id).is(":visible")) {
                thisO._objects[zoom].trigger('click');
            //}
        });
 ...
 */
searchResult.prototype.hideTooltip = function(zoom) {
    var thisO = this;
 ...
 *
 */
searchResult.prototype.dim = function(zoom) {
    var thisO = this;
 ...
 *
 */
searchResult.prototype.undim = function(zoom) {
    var thisO = this;
...
switch (window.scenario) {
    ...
    default:
        /*
        window.searchResultData['chacaito']['data']['distanceToMe'] = ["2 Km", 2000];
        window.searchResultData['ccchacao']['data']['distanceToMe'] = ["1.8 Km", 1800];
        window.searchResultData['campoalegre']['data']['distanceToMe'] = ["1.6 Km", 1700];
        window.searchResultData['ccsanignacio']['data']['distanceToMe'] = ["450 m", 450];
        window.searchResultData['chacao']['data']['distanceToMe'] = ["700 m", 700];
        window.searchResultData['bellocampo']['data']['distanceToMe'] = ["800 m", 800];
        window.searchResultData['lacastellana']['data']['distanceToMe'] = ["120 m", 120];
        window.searchResultData['plazaaltamira']['data']['distanceToMe'] = ["400 m", 300];;
        window.searchResultData['altamira']['data']['distanceToMe'] = ["150 m", 150];
        window.searchResultData['plazapalosgrandes']['data']['distanceToMe'] = ["1 Km", 1000];
        window.searchResultData['parquedeleste']['data']['distanceToMe'] = ["2.2 Km", 2200];
        */
